# Trench Crusade — Glossário Oficial PT-BR

Referência canônica de tradução para criação de conteúdo homebrew. Todos os termos aqui estabelecidos devem ser usados de forma consistente em warbands, cenários, relatos e lore.

---

## 1. ATRIBUTOS (Stats)

| Inglês | PT-BR | Abreviação | Descrição |
| :--- | :--- | :---: | :--- |
| Movement | Movimento | **M** | Distância em polegadas por ação. |
| Melee | Corpo a Corpo | **CaC** | Bônus de dados em combate próximo. |
| Ranged | Distância | **Dist** | Bônus de dados em ataques à distância. |
| Armour | Armadura | **Arm** | Modificador subtraído da rolagem de ferimento recebida. |
| Speed | Velocidade | **V** | Usado em testes de iniciativa e esquiva. |
| Base Size | Tamanho de Base | — | Diâmetro da base do modelo (25mm, 32mm, 40mm). |

---

## 2. PALAVRAS-CHAVE DE MODELO (Model Keywords)

| Inglês (ALL CAPS) | PT-BR (ALL CAPS) | Efeito Resumido |
| :--- | :--- | :--- |
| LEADER | LÍDER | Único por warband. Bônus de moral para aliados próximos. |
| ELITE | ELITE | Ganha XP, pode ser promovido, tem Rolagem de Trauma. |
| FOLLOWER | SEGUIDOR | Infantaria básica. Sem XP, sem Rolagem de Trauma. |
| TOUGH | RESISTENTE | Ao receber Abatido, role novamente: em 4+ trata como Marcador de Sangue. |
| STRONG | FORTE | Usa armas e itens com PESADO sem penalidade. |
| FEAR | MEDO | Inimigos que queiram atacar este modelo fazem teste de moral primeiro. |
| FLYING | VOADOR | Ignora terreno ao mover. Não pode ser carregado por modelos sem VOADOR. |
| GOLEM | GOLEM | Imune a MEDO, Veneno e GÁS. Sem Rolagem de Trauma. |
| INFILTRATOR | INFILTRADOR | Pode ser colocado em qualquer ponto do campo antes da partida. |
| SKIRMISHER | ESCARAMUÇADOR | Move e atira sem penalidade. Recua do corpo a corpo sem ataque gratuito. |
| ASSAULT | ASSALTO | Move e atira na mesma ativação. Pode atirar após Carga. |
| DEADLY | LETAL | Em Crítico, adiciona +1 dado extra à Rolagem de Ferimento. |
| REGENERATE | REGENERAR | Remove 1 Marcador de Sangue no início de cada ativação. |
| HEAVY | PESADO | Requer FORTE para usar sem penalidade; senão, -1 ação por turno. |

---

## 3. PALAVRAS-CHAVE DE ARMA / ATAQUE (Weapon Keywords)

| Inglês (ALL CAPS) | PT-BR (ALL CAPS) | Efeito Resumido |
| :--- | :--- | :--- |
| AUTOMATIC(X) | AUTOMÁTICO(X) | Em sucesso, role X Rolagens de Ferimento extras contra o mesmo alvo. |
| BLAST(X") | EXPLOSÃO(X") | Área de efeito com raio X". Todos dentro sofrem Rolagem de Ferimento. |
| RELOAD | RECARREGAR | Após disparar, gasta 1 ação para recarregar antes de usar novamente. |
| PISTOL | PISTOLA | Pode ser usada em corpo a corpo sem penalidade. Não requer 2 mãos. |
| BAYONET LUG | SUPORTE DE BAIONETA | A arma pode receber baioneta; usada como arma corpo a corpo básica. |
| FOCUSED FIRE | FOGO CONCENTRADO | 2+ modelos com FOGO CONCENTRADO no mesmo alvo somam dados de sucesso. |
| SHRAPNEL | ESTILHAÇO | Em EXPLOSÃO, modelos na área recebem Marcadores de Sangue extras em falhas. |
| FLAMETHROWER | LANÇA-CHAMAS | Área em linha. Ignora Cobertura. Aplica FOGO mesmo em "Sem Efeito". |
| FIRE | FOGO | Alvo fica Em Chamas: Rolagem de Ferimento automática no início de cada ativação. |
| GÁS | GÁS | Aplica Gaseado: Penalidade em todos os testes; Marcador de Sangue extra por turno. |
| IGNORE ARMOUR | IGNORA ARMADURA | A Armadura do alvo não é aplicada à Rolagem de Ferimento. |
| CRITICAL | CRÍTICO | Em Sucesso Crítico, role +2 dados na Rolagem de Ferimento em vez de +1. |
| RISKY | ARRISCADO | Em falha, o atirador também sofre uma Rolagem de Ferimento. |
| DESPATCH | EXECUÇÃO | Se o alvo estiver Abatido, ignora a Armadura automaticamente. |
| INFECTION MARKER | MARCADOR DE INFECÇÃO | Ao ferir, coloca marcador que reduz atributos até o fim da batalha. |
| CLEAVE(X) | GOLPE VARRENTE(X) | Se o ataque remove um modelo, X de dano excedente atinge outro adjacente. |
| OTHISMOS | ARREMESSO | Empurra o alvo D6" na direção do ataque após acerto bem-sucedido. |
| BATTER | PORRADA | A arma pode ser usada em corpo a corpo com bônus de CaC adicional. |
| ASSAULT | ASSALTO | Pode mover e atacar à distância na mesma ativação sem penalidade. |

---

## 4. ESTADOS DE CONDIÇÃO (Condition States)

| Inglês | PT-BR | Como é Adquirido | Efeito |
| :--- | :--- | :--- | :--- |
| Down | Abatido | Rolagem de Ferimento 7–8. | Não pode agir. Aliados podem levantar (1 ação). Inimigos podem executar. |
| Out of Action | Fora de Ação | Rolagem de Ferimento 9+, ou 6 Marcadores de Sangue. | Removido do campo. Elites rolam Trauma. |
| On Fire | Em Chamas | Armas com FOGO ou LANÇA-CHAMAS. | Rolagem de Ferimento automática no início de cada ativação. |
| Gassed | Gaseado | Armas com GÁS. | Penalidade em todos os testes; Marcador de Sangue extra por turno. |
| Infected | Infectado | Armas com MARCADOR DE INFECÇÃO. | -1 permanente em atributo por marcador até o fim da batalha. |
| Feared | Aterrorizado | Falha em teste de moral contra MEDO. | Não pode atacar o causador do medo neste turno. |

---

## 5. SISTEMA DE MARCADORES

| Inglês | PT-BR | Símbolo | Regras |
| :--- | :--- | :---: | :--- |
| Blood Marker | Marcador de Sangue | 🩸 | Máx. 6. Cada um: +1 à Rolagem de Ferimento contra o modelo. 6 = Fora de Ação automático. |
| Blessing Marker | Marcador de Bênção | ✨ | Máx. 6. Gaste 1 para: +DADO em qualquer teste OU remover 1 Marcador de Sangue. |
| Infection Marker | Marcador de Infecção | ☣ | Cada um: -1 em um atributo até o fim da batalha. Removido com Água Benta. |
| Glory Points | Pontos de Glória | ☼ | Moeda secundária para mercenários e itens especiais de campanha. |

---

## 6. SISTEMA DE ROLAGEM

| Inglês | PT-BR | Descrição |
| :--- | :--- | :--- |
| Success Roll | Rolagem de Sucesso | 2D6: 2–6 Falha / 7–11 Sucesso / 12+ Crítico. |
| Injury Roll | Rolagem de Ferimento | 2D6: 1 Sem Efeito / 2–6 Marcador de Sangue / 7–8 Abatido / 9+ Fora de Ação. |
| Bloodbath Roll | Rolagem de Banho de Sangue | 3D6 em Rolagem de Ferimento ao gastar 6 Marcadores de Sangue (ou 3 se Abatido). |
| Trauma Roll | Rolagem de Trauma | D66 pós-batalha para cada ELITE colocado Fora de Ação. |
| Advancement Roll | Rolagem de Avanço | Rolagem 2D6 em tabela de habilidades ao atingir marco de XP. |
| Success Dice (+DICE) | +DADO | Role dados extras; escolha os 2 melhores resultados. |
| Penalty Dice (-DICE) | -DADO (Penalidade) | Role dados extras; escolha obrigatoriamente os 2 piores resultados. |
| D66 | D66 | Role 2D6: um dado = dezenas (1–6), outro = unidades (1–6). Resultados: 11–66. |

---

## 7. AÇÕES E ATIVAÇÃO

| Inglês | PT-BR | Descrição |
| :--- | :--- | :--- |
| Activation | Ativação | Momento em que um modelo age durante a Fase de Ativação. |
| Move | Mover | O modelo se move até M" em qualquer direção. |
| Dash | Correr | Move até 2×M" mas não pode atacar neste turno. |
| Charge | Carregar | Move em direção a um inimigo e entra em corpo a corpo. |
| Retreat | Recuar | Sai do corpo a corpo; o oponente recebe um ataque gratuito. |
| Shoot | Atirar | Realiza ataque à distância com arma equipada. |
| Fight | Lutar | Realiza ataque corpo a corpo contra modelo em contato. |
| Line of Sight (LOS) | Linha de Visão (LV) | Visibilidade direta entre dois modelos. |
| Engagement Range | Zona de Engajamento | 1" ao redor da base de qualquer modelo. Entrar = engajamento obrigatório. |

---

## 8. TERRENO

| Inglês | PT-BR | Efeito |
| :--- | :--- | :--- |
| Open | Aberto | Sem penalidade de movimento. |
| Difficult | Difícil | Conta como 2" por 1" movido. |
| Dangerous | Perigoso | Ao entrar, teste de Velocidade. Falha = Rolagem de Ferimento automática. |
| Impassable | Intransponível | Não pode ser atravessado. |
| Light Cover | Cobertura Leve | -1 ao ataque à distância. |
| Heavy Cover | Cobertura Pesada | -DADO (Penalidade) ao ataque à distância. |
| No Man's Land | Terra de Ninguém | Campo aberto com crateras e arame farpado. |
| Trench Lines | Linhas de Trincheira | Redes de valas; Cobertura Pesada para quem está dentro. |

---

## 9. REGRAS DE CAMPANHA

| Inglês | PT-BR | Descrição |
| :--- | :--- | :--- |
| Campaign Phase | Fase de Campanha | Sequência pós-batalha com 5 etapas obrigatórias. |
| Trauma Step | Etapa de Trauma | Rolagem D66 para cada ELITE removido como Fora de Ação. |
| Promotions & Experience Step | Etapa de Promoção e Experiência | ELITEs sobreviventes ganham XP e avançam. |
| Reinforcements Step | Etapa de Reforços | Recrutamento de novos modelos com Ducats. |
| Exploration Step | Etapa de Exploração | Rolagem em tabela para recursos, relíquias ou eventos. |
| Quartermaster Step | Etapa do Intendente | Compra, venda e designação de equipamentos. |
| Glorious Deed | Feito Glorioso | Ação notável que concede +1 XP e/ou +1 Ponto de Glória. |
| Advancement Milestone | Marco de Avanço | Quantidade de XP que aciona uma Rolagem de Avanço. |
| Field Strength | Força de Campo | Número máximo de modelos que uma warband leva por jogo. |
| Warband Threshold | Limiar da Warband | Tabela de orçamento e Field Strength por número de jogo. |

---

## 10. MOEDAS E RECURSOS

| Inglês | PT-BR | Símbolo | Uso |
| :--- | :--- | :---: | :--- |
| Ducats | Ducados | 👑 | Moeda principal. Compra modelos, armas e equipamentos. |
| Glory Points | Pontos de Glória | ☼ | Moeda secundária. Mercenários e itens raros de campanha. |
| Experience Points | Pontos de Experiência | XP | Progressão de ELITEs. |

---

## 11. TIPOS DE MODELO

| Inglês | PT-BR | Regras |
| :--- | :--- | :--- |
| ELITE / LEADER | ELITE / LÍDER | Único LÍDER por warband. Ganha XP, Rolagem de Trauma, pode avançar. |
| ELITE | ELITE | Modelo nomeado com XP e progressão. Tem Rolagem de Trauma. |
| Follower | Seguidor | Infantaria básica. Sem XP, sem progressão, sem Rolagem de Trauma. |
| Hired Sword / Mercenary | Mercenário | Contratado com Pontos de Glória. Sem XP, sem Rolagem de Trauma. |

---

## 12. PATRONOS (Patrons)

| Inglês | PT-BR | Alinhamento | Bônus Principal |
| :--- | :--- | :--- | :--- |
| Temporal Lord | Senhor Temporal | Fiéis | Ducados extras por vitória. Bônus no recrutamento de Seguidores. |
| Warrior Saint | Santo Guerreiro | Fiéis | +1 DADO em ataques corpo a corpo por batalha. |
| Learned Saint | Santo Estudioso | Fiéis | Rola 3 tabelas de habilidade (em vez de 2) em Rolagens de Avanço. |
| Infernal Noble | Nobre Infernal | Heréticos | Mercenários Heréticos com desconto em Pontos de Glória. |
| Sublime Gate | Portal Sublime | Heréticos | Um modelo pode se teletransportar 6" uma vez por batalha. |
| Order of the Fly | Ordem da Mosca | Black Grail | Marcadores de Infecção causam -2 a atributos (em vez de -1). |
| Mammon | Mammon | Neutro | Ducados extras na Etapa de Exploração; acesso a itens raros. |
| Antipope of Avignon | Antipapa de Avignon | Peregrinos | Martírio concede Marcadores de Bênção em vez de Sangue para aliados. |

---

## 13. FACÇÕES

| Inglês | PT-BR | Alinhamento |
| :--- | :--- | :--- |
| Principality of New Antioch | Principado de Nova Antioquia | Fiel |
| Iron Sultanate | Sultanato de Ferro | Fiel |
| Trench Pilgrims | Peregrinos das Trincheiras | Fiel (heterodoxo) |
| Heretic Legion | Legião Herética | Herético |
| Cult of the Black Grail | Culto do Graal Negro | Herético |
| Court of the Seven-Headed Serpent | Corte da Serpente de Sete Cabeças | Herético |

---

## 14. LOCAIS

| Inglês | PT-BR |
| :--- | :--- |
| No Man's Land | Terra de Ninguém |
| New Antioch | Nova Antioquia |
| Iron Wall | Muralha de Ferro |
| The Abyss / Abyssal Gate | O Abismo / Portão Abissal |
| Avignon | Avignon |
| Jerusalem | Jerusalém |
| Constantinople | Constantinopla |
| Decimated Ruins | Ruínas Decimadas |
| Trench Lines | Linhas de Trincheira |

---

## 15. CONCEITOS DE LORE

| Inglês | PT-BR | Descrição |
| :--- | :--- | :--- |
| The Great Heresy | A Grande Heresia | Evento de 1099; abertura do Portão Abissal sob o Templo de Salomão. |
| Abyssal Gate | Portão Abissal | Portal permanente para o Inferno aberto pelos Templários. |
| Trench Saints | Santos das Trincheiras | Soldados comuns que manifestam milagres sob estresse extremo. |
| Antipope | Antipapa | Líder herético dos Peregrinos; excomungado pelo Papa de Nova Antioquia. |
| Orichalcum | Orichalcum | Metal mágico raro usado em armaduras e equipamentos especiais. |
| Greek Fire | Fogo Grego | Substância incendiária de origem demoníaca; arde na água. |
| Goetia | Goetia | Magia demoníaca baseada em grimórios infernais. |
| Golem | Golem | Construto artificial animado; tecnologia judaico-islâmica repropositada. |
| Mubarizon | Mubarizon | Combate singular ritualístico do Sultanato de Ferro. |
| Mustard Gás | Gás Mostarda | Arma química da Grande Guerra; versão infernal causa efeitos abissais. |

---

## 16. CENÁRIOS — NOMES

| Inglês | PT-BR |
| :--- | :--- |
| Claim No Man's Land | Reclamar a Terra de Ninguém |
| Hunt for Heroes | Caça aos Heróis |
| Relic Hunt | Caça à Relíquia |
| Trench Warfare | Guerra de Trincheiras |
| Armoured Train | Trem Blindado |
| Dragon Hunt | Caça ao Dragão |
| Supply Raid | Saque de Suprimentos |
| From Below | Vindo de Baixo |
| Fields of Glory | Campos de Glória |
| Don't Breathe | Não Respire |
| The High Ground | Terreno Elevado |
| Great War | A Grande Guerra |

---

## 17. TABELAS DE HABILIDADE — NOMES

| Inglês | PT-BR |
| :--- | :--- |
| Melee & Strength | Corpo a Corpo e Força |
| Ranged & Marksmanship | Distância e Pontaria |
| Stealth & Speed | Furtividade e Velocidade |
| Wildcard | Coringa |
| Berserker | Berserker |
| Iron Grip | Punho de Ferro |
| Brutal Strike | Golpe Brutal |
| Counter-Attack | Contra-Ataque |
| Tough as Nails | Duro de Matar |
| Cleave | Golpe Varrente |
| Relentless | Implacável |
| Bloodlust | Sede de Sangue |
| Giant Killer | Mata-Gigantes |
| Warlord | Senhor da Guerra |
| Crack Shot | Atirador de Elite |
| Eagle Eye | Olho de Águia |
| Steady Aim | Mira Firme |
| Quick Draw | Saque Rápido |
| Suppressing Fire | Fogo de Supressão |
| Pinpoint | Precisão Cirúrgica |
| Double Tap | Tiro Duplo |
| Deadeye | Olho Certeiro |
| Ghost | Fantasma |
| Swift | Ágil |
| Acrobat | Acrobata |
| Shadow | Sombra |
| Ambusher | Emboscador |
| Sprint | Arrancada |
| Evasion | Evasão |
| Disappear | Desaparecer |
| Iron Will | Vontade de Ferro |
| Fearless | Destemido |
| Resourceful | Engenhoso |
| Surgeon | Cirurgião |
| Blessed | Abençoado |
| Veteran | Veterano |
| Tough | Resistente |
| Regenerate | Regenerar |
| Fearmonger | Semeador do Medo |
| Champion | Campeão |

---

## 18. TABELA DE TRAUMA — RESULTADOS

| Inglês | PT-BR |
| :--- | :--- |
| Dead | Morto |
| Near Death | Às Portas da Morte |
| Leg Wound | Ferimento na Perna |
| Expensive Treatment | Tratamento Caro |
| Possessed | Possuído |
| Muscle Damage | Dano Muscular |
| Minor Wound | Ferimento Leve |
| Robbed | Roubado |
| Full Recovery | Recuperação Total |
| Hardened | Endurecido |
| Hardened by Battle | Temperado em Batalha |

---

## 19. EQUIPAMENTOS — NOMES

| Inglês | PT-BR |
| :--- | :--- |
| Field Dressing | Curativo de Campo |
| Gás Mask | Máscara de Gás |
| Holy Relic | Relíquia Sagrada |
| Iron Idol | Ídolo de Ferro |
| Satchel Charge | Carga Explosiva |
| Sniper Scope | Mira de Precisão |
| Stimulants | Estimulantes |
| Trench Map | Mapa de Trincheiras |
| War Banner | Estandarte de Guerra |
| Alchemical Ammunition | Munição Alquímica |
| Angelic Instrument | Instrumento Angélico |
| Binoculars | Binóculos |
| Battle Standard | Estandarte de Batalha |
| Saint's Finger Bone | Falange do Santo |
| Daemon's Eye | Olho do Demônio |
| Infernal Contract | Contrato Infernal |
| Holy Water Flask | Frasco de Água Benta |

---

## REGRAS DE USO

1. **Palavras-chave em ALL CAPS** em todos os contextos mecânicos, sempre em português.
2. **Alcance sempre em polegadas** com `"` — ex: `6"`, `24"`, `48"`.
3. **Custo em 👑 (Ducados) ou ☼ (Pontos de Glória)** — nunca "pts" ou "pontos".
4. **Habilidades com nome em itálico** — *Nome da Habilidade*: efeito.
5. **Tipos de modelo**: ELITE/LÍDER, ELITE, Seguidor — sem outros.
6. **Condições** sempre pelo nome PT-BR: Abatido, Fora de Ação, Em Chamas, Gaseado.
7. **Em caso de dúvida**, consulte a coluna PT-BR deste glossário antes de criar um novo termo.
